...
class Result {
  constructor(public readonly success: string, public readonly error: string) {}
}
...
function analyzeArray(array: Array<any>, options: Options): string {
  ...
  const typeName = analyzeArrayProperty(
    "nested",
    array,
    collector,
    inlineShift
...
function analyzeArrayProperty(
  ...
  return `repeated ${analyzeObjectProperty(
    key,
    first,
    collector,
    inlineShift
...
function mergePrimitiveType(
  a: ProtoPrimitiveType,
  b: ProtoPrimitiveType
): ProtoPrimitiveType {
  if (a.name === b.name) {
    return a;
  }
